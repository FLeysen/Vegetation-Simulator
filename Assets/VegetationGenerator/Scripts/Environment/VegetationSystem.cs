...
namespace VegetationGenerator
{
    ...
    {
        [SerializeField] private bool _allowReset = true;
        [SerializeField] private KeyCode _resetButton = KeyCode.R;
        [SerializeField] private float _resetHoldTime = 3f;
        ...
        private void Update()
        {
            if (!Input.GetKey(_resetButton) || !_allowReset)
            {
                _resetHoldDuration = 0f;
                return;
            }

            if ((_resetHoldDuration += Time.deltaTime) > _resetHoldTime)
            {
                ResetVegetation();
                _resetHoldDuration = 0f;
            }
        ...
        private void ResetVegetation()
        {
            foreach (Plant plant in _plants)
                Destroy(plant.gameObject);
            _plants.Clear();

            GetComponent<TrackAndPassTime>().ResetToZero();
            GetComponent<SeasonChanger>().ResetToStart();
            GetComponent<AutomaticTimePassage>().ForcePause();
